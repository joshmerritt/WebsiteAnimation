...
const STEPS = [
  { key: 'clicks',   label: 'Click',  desc: 'Ball touched / dragged' },
...
export default function FunnelViz({ totals }) {
  ...
  const maxVal = totals.clicks;
  const overallConv = ((totals.opens / totals.clicks) * 100).toFixed(1);
  ...
  return (
    <div className="funnel-viz">
      <svg width="100%" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="xMidYMid meet">
        ...
        {STEPS.map((step, i) => {
          const val = totals[step.key];
          const barW = (val / maxVal) * maxWidth;
          const nextVal = i < STEPS.length - 1 ? totals[STEPS[i + 1].key] : val * 0.85;
        ...
        <text x={cx} y={h - 6} textAnchor="middle" className="funnel-overall">
          {overallConv}% overall click-to-open conversion
        </text>
      </svg>
    </div>
  );
