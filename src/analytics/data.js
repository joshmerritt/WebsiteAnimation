...
export function generateTimeSeriesData(days = 90) {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    // Organic growth pattern — portfolio sites build slowly
    const base = isWeekend ? 18 : 32;
    const trend = Math.max(0, (days - i) * 0.25);
    const noise = (rand() - 0.5) * 20;
    // Occasional spikes from LinkedIn posts / shares
    ...
    data.push({
      ...
      avgDuration: Math.round(35 + rand() * 200),     // seconds on site
      bounceRate: Math.round(25 + rand() * 40),        // % single-page visits
      ballInteractions: Math.round(visitors * (0.4 + rand() * 0.3)),  // unique users who drag a ball
    });
  }
  return data;
}
...
// ── Traffic sources (realistic for a portfolio site) ────────────────────
...
// ── Top pages (matches actual project detail routes) ────────────────────
...
// ── Ball engagement funnel (matches actual 8 project balls) ─────────────
// clicks = user touched/dragged the ball
// launches = ball was released with enough power
// scores = ball hit the matching goal (collision)
// opens = detail modal was displayed
