...
function generateTimeSeriesData(days = 90) {
  const now = new Date(); const data = []; const rand = seededRandom(42);
  for (let i = days - 1; i >= 0; i--) {
    ...
    const visitors = Math.round(Math.max(5, base + trend + noise + spike));
    const ballInteractions = Math.round(visitors * (0.4 + rand() * 0.3));
    const shots = Math.round(ballInteractions * (1.8 + rand() * 1.5));
    const makes = Math.round(shots * (0.75 + rand() * 0.15));
    const opens = Math.round(makes * (0.85 + rand() * 0.1));
    const ctaClicks = Math.round(opens * (0.25 + rand() * 0.2));
    data.push({
      date: date.toISOString().split("T")[0],
      label: date.toLocaleDateString("en-US", { month: "short", day: "numeric" }),
      dayOfWeek: date.getDay(),
      visitors, pageviews: Math.round(visitors * (1.8 + rand() * 1.4)),
      avgDuration: Math.round(35 + rand() * 200), bounceRate: Math.round(25 + rand() * 40),
      ballInteractions, shots, ctaClicks,
    });
  }
  return data;
}
...
const BALL_ENGAGEMENT = [
  { ball: "Josh Merritt", id: "aboutMe", clicks: 312, launches: 289, scores: 267, opens: 251, ctaClicks: 89, color: "#6B9F6B", category: "Me" },
  { ball: "Microsoft Power BI", id: "powerBIMetrics", clicks: 234, launches: 218, scores: 189, opens: 178, ctaClicks: 72, color: "#D4A843", category: "Business" },
  { ball: "The Wine You Drink", id: "thewineyoudrink", clicks: 198, launches: 182, scores: 156, opens: 149, ctaClicks: 63, color: "#8B1A32", category: "Apps" },
  { ball: "Black Sheep Dart League", id: "dartleague", clicks: 178, launches: 165, scores: 141, opens: 132, ctaClicks: 48, color: "#7B5EA7", category: "Apps" },
  { ball: "Smart Chicken Coop", id: "arduinoCoopDoor", clicks: 156, launches: 142, scores: 122, opens: 108, ctaClicks: 35, color: "#BF360C", category: "Technology" },
  { ball: "Site Analytics", id: "SiteAnalytics", clicks: 145, launches: 131, scores: 112, opens: 98, ctaClicks: 31, color: "#5985B1", category: "Technology" },
  { ball: "Google Data Studio", id: "googleDataStudioServiceTechs", clicks: 123, launches: 108, scores: 89, opens: 78, ctaClicks: 24, color: "#4285F4", category: "Business" },
  { ball: "Portfolio Website", id: "thisWebsite", clicks: 108, launches: 95, scores: 84, opens: 72, ctaClicks: 22, color: "#5985B1", category: "Technology" },
];

const SESSION_FLOW = [
  { from: "Landing", to: "Ball Interaction", pct: 63, color: "#5985B1" },
  { from: "Landing", to: "Bounce", pct: 32, color: "#C05050" },
  { from: "Landing", to: "Scroll Only", pct: 5, color: "#D4A843" },
  { from: "Ball Interaction", to: "Detail Page", pct: 48, color: "#6B9F6B" },
  { from: "Ball Interaction", to: "Multiple Balls", pct: 12, color: "#7B5EA7" },
  { from: "Ball Interaction", to: "Exit", pct: 3, color: "#C05050" },
];

const DEVICE_DATA = [
  { device: "Desktop", icon: "\uD83D\uDDA5\uFE0F", pct: 60.3, interactionRate: 72, avgShots: 4.8, accuracy: 68, avgDuration: 142 },
  { device: "Mobile", icon: "\uD83D\uDCF1", pct: 34, interactionRate: 51, avgShots: 2.1, accuracy: 45, avgDuration: 78 },
  { device: "Tablet", icon: "\uD83D\uDCBB", pct: 5.7, interactionRate: 82, avgShots: 6.2, accuracy: 71, avgDuration: 165 },
];
...
// ═══ ANALYTICS TAB ══════════════════════════════════════════════════════
function AnalyticsTab({ timeSeriesData, rangeDays }) {
  ...
  return (<div>
    ...
    <div style={{ marginTop: 24, paddingTop: 14, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
      <p style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", lineHeight: 1.7 }}>Real-time analytics powered by a custom GA4 Data API pipeline — a Cloudflare Worker authenticates via service account, queries the GA4 reporting endpoint, and returns structured JSON that this React dashboard consumes. Device breakdown, session flow, and architecture sections use supplementary demonstration data.</p>
    </div>
  </div>);
}
...
export default function AnalyticsDashboardV3() {
  ...
  return (
    <div style={SS.root}>
      ...
      <header style={SS.header}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={SS.liveBadge}><span style={SS.liveDot} />Live</div>
      ...
      {activeTab === "analytics" ? <AnalyticsTab timeSeriesData={timeSeriesData} rangeDays={rangeDays} /> : <DataArchitectureTab />}
    </div>
  );
}
